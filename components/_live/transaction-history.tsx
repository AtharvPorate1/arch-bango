    ...
    <div className="space-y-4 dm-sans">
      {transactionHistory.length > 0 ? (
          ...
          <Card key={transaction.id} className="bg-[#1c1f2e]/50 border-none cursor-pointer">
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div
                  className={`p-2 rounded-full ${transaction.order_type === "BUY" ? "bg-green-500/20" : "bg-red-500/20"}`}
                >
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400">
                        {transaction.createdAt.split("T")[0]} {transaction.createdAt.split("T")[1].split(":")[0]}:
              ...
              <div className="text-right">
                <p
                  className={`text-sm font-medium ${transaction.order_type === "BUY" ? "text-green-500" : "text-red-500"}`}
                >
                  {transaction.order_type === "BUY" ? "+" : "-"} {transaction.order_type === "BUY" ? "$" : "Shares"}{" "}
                  {transaction.order_type === "BUY" ? transaction.amount : transaction.order_size}
                </p>
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <div className="bg-[#1c1f2e]/50 p-4 rounded-full mb-4">
            <History className="w-8 h-8 text-gray-400" />
          </div>
          <h3 className="text-lg font-medium text-gray-200">No transactions yet</h3>
